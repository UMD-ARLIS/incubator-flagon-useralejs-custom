...
var logs$1;
var config$2; // Interval Logging Globals
...
function packageLog(e, detailFcn) {
  ...
  var timeFields = extractTimeFields(e.timeStamp && e.timeStamp > 0 ? config$2.time(e.timeStamp) : Date.now());
  var log = {
    ...
    'userAction': true,
    'details': details,
    'userId': config$2.userId,
    'toolVersion': config$2.version,
    'toolName': config$2.toolName,
    'useraleVersion': config$2.useraleVersion,
    'sessionID': config$2.sessionID
  };
...
function packageIntervalLog(e) {
  ...
  var type = e.type;
  var timestamp = Math.floor(e.timeStamp && e.timeStamp > 0 ? config$2.time(e.timeStamp) : Date.now()); // Init - this should only happen once on initialization
...
}(userale2_3_0, userale2_3_0.exports));
...
var targetNode = document; // Options for the observer (which mutations to observe)
...
var callback = function callback(mutationList, observer) {
  mutationList = mutationList.map(function (mutation) {
    return {
      type: mutation.type,
